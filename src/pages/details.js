...
const phoneName = getParameterByName('ph');
const phone = phones.filter((val) => val.name === phoneName)[0];
  ...
  return (
    <Layout>
      <PhoneLayout>
        ...
        <div>${pricing[Object.keys(pricing)[0]].plan || "49"} per month</div>
        <ResponsiveButton onClick={ (e) => { addItem(phone.brand + ' ' + phone.name)} }>Add to cart</ResponsiveButton>
      </PhoneLayout>
    </Layout>
  );
};
